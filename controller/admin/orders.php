<?php
  ...
  if( route(2)  ==  "counter" ):
    ...
    echo '<li'; if( !$active ): echo ' class="active"'; endif; echo '>
            <a href="/admin/orders/all">Todos os pedidos ('.$count.')</a>
          </li>';
    ...
    if( $_POST ):
        ...
        elseif( route(2) == "set_partial" ):
          ...
          if( empty($remains) || !is_numeric($remains) ):
            $error      = 1;
            $errorText  = "O valor não entregue não pode estar vazio";
            $icon       = "error";
          elseif( $order["order_quantity"] < $remains ):
            $error      = 1;
            $errorText  = "A quantidade de saída não pode exceder a quantidade do pedido.";
            $icon       = "error";
          else:
