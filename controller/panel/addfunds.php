...
if ($_POST && $_POST["payment_bank"]):
    ...
    if (open_bankpayment($user["client_id"]) >= 2) {
        unset($_SESSION["data"]);
        $error = 1;
        $errorText = 'Você tem 2 notificações de pagamento com aprovação pendente, você não pode criar novas notificações.';
    } elseif (empty($bank)) {
        $error = 1;
        $errorText = 'Selecione uma conta bancária válida.';
    } elseif (!is_numeric($amount)) {
        $error = 1;
        $errorText = 'Por favor, insira um valor válido.';
    } elseif (empty($gonderen)) {
        $error = 1;
        $errorText = 'Insira um nome de remetente válido.';
    } else {
        ...
        if ($insert) {
            ...
            if ($settings["alert_newbankpayment"] == 2):
                ...
                if ($sendsms):
                    SMSUser($settings["admin_telephone"], "Nova solicitação de pagamento criada em seu site e ID é: #" . $conn->lastInsertId());
                endif;
                if ($sendmail):
                    sendMail(["subject" => "Novo pedido de pagamento", "body" => "Nova solicitação de pagamento criada em seu site e ID é: #" . $conn->lastInsertId(), "mail" => $settings["admin_mail"]]);
                endif;
            endif;
        } else {
            $error = 1;
            $errorText = 'Ocorreu um erro durante o envio do alerta, tente novamente mais tarde..';
        }
        ...
        if (empty($method_id)) {
            ...
            $error = 1;
            $errorText = 'Valor mínimo de pagamento ' . $settings["csymbol"] . $method["method_min"];
        } elseif ($amount > $method["method_max"] && $method["method_max"] != 0) {
            $error = 1;
            $errorText = 'Valor máximo de pagamento ' . $settings["csymbol"] . $method["method_max"];
        } else {
            if ($method_id == 1):
        ...
        elseif ($method_id == 16) :
            ...
            elseif ($method_id == 2):
                ...
                if ($insert):
                    ...
                    $error = 1;
                    $errorText = "Ocorreu um erro ao iniciar o seu pagamento, tente novamente mais tarde..";
                endif;
            elseif ($method_id == 8):
            ...
            elseif ($method_id == 19):
                ...
                MercadoPago\SDK::setAccessToken($access_token);
                  ...
                  <div class="modal fade" id="pix">
                    <div class="modal-dialog">
                    <div class="modal-content">
                     <!-- Modal Header -->
                          <div class="modal-header">
                            <h4 class="modal-title" style="font-weight: 600; font-size: 18px;">Escanei o código QR a baixo para realizar o pagamento...</h4>
                                </div>
                                <!-- Modal body -->
                                <div class="modal-body">
                                <div class="card-box-top" style="color: #2a255c; padding: 20px;border-radius: 10px; margin: 0 0 25px 0; background: rgba(42, 37, 92, 0.15)">
                               <img src="data:image/jpg;base64,'.$payment->point_of_interaction->transaction_data->qr_code_base64.'" id="img-pix" class="img-thumbnail" width="100%" alt="">
                               </div>
                                  <p style="font-weight: 600; text-align: center; font-size: 18px;">Você também pode copiar o código abaixo:</p>
                                  <ul>
                                  <li>Clique no botão abaixo para copiar o código</li>
                                  <li>No APP do seu banco procure a opção <b>"PIX COPIA E COLA"</b></li>
                                  <li>Cole o código lá e efetue o pagamento</li>
                                  <li>Após, volte ao site que seu saldo estará disponível.</li>
                                  </ul>
                                  <center>
                                    <textarea id="codigo-pix" style="width: 100%;height: 70px;" rows="8" class="form-control" name="" rows="8" cols="80">'.$payment->point_of_interaction->transaction_data->qr_code.'</textarea>
                                  <button onclick="$(\'#codigo-pix\').select();document.execCommand(\'copy\');" style="margin-top:10px;" type="button" class="btn btn-primary" name="button">
                                    Copiar Código
                                  </button>
                                  <br><br>
                                  <button onClick="window.location.href=window.location.href" type="button" class="btn btn-secondary" data-dismiss="modal">FECHAR</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    if ($payment_url):
        echo '<script>setInterval(function(){window.location="' . $payment_url . '"},1000)</script>';
    endif;
